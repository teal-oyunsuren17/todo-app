...
const TaskCard = ({ item, index, getDatas }: any) => {
  const [edit, setEdit] = React.useState(false)
  const [done, setDone] = React.useState(item.done)
  const [editedList, setEditedList] = React.useState(item.list)
  ...
  function handleDone(checked: boolean, id: any) {
    setDone(!done)
    if (checked) {
      axios
  ...
  const handleKey = (event: any, id: any) => {
    if (event.key === "Enter") {
      handleEdit(id)
    }
  }
